...
-spec b_hold(kapps_call:call()) ->
          kapps_api_std_return().
b_hold(Call) -> b_hold(?MAX_WAIT, 'undefined', Call).
...
b_record_call(MediaName, Call) ->
    ...
    wait_for_headless_application(<<"record">>, <<"RECORD_STOP">>, <<"call_event">>, ?MAX_WAIT).
...
wait_for_say(Call) ->
    wait_for_message(Call, <<"say">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"call_event">>, ?MAX_WAIT).
...
wait_for_bridge(Timeout, Fun, Call, Start, {'ok', JObj}) ->
    ...
    case get_event_type(JObj) of
        ...
        {<<"call_event">>, <<"CHANNEL_BRIDGE">>, _} ->
            ...
            wait_for_bridge(?MAX_WAIT, Fun, Call);
        {<<"call_event">>, <<"CHANNEL_DESTROY">>, _} ->
...
wait_for_noop(Call, NoopId) ->
    wait_for_noop(Call, NoopId, ?MAX_WAIT).
...
wait_for_channel_bridge() ->
    case receive_event(?MAX_WAIT) of
        {'ok', JObj}=Ok ->
            case kz_util:get_event_type(JObj) of
